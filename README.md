...
全课 38 步。每个提问单独成一步，画面停在问题上，右上角标出学生此时的活动（猜一猜、小组交流、动手拨一拨、想一想……），下一步才给出明确结论；连续进位（4、8）放慢展示。整节课由一条 [GSAP](https://gsap.com) 时间线驱动。点击画面或按方向键逐步讲解，每一步下方会显示取自教参的教学提示（追问、组织建议）；也可以点「自动播放」连续播放。GSAP 的核心和插件放在 `vendor/gsap/`，离线也能打开。
